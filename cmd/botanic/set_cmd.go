...
func setCmd(rootConfig *rootCmdConfig) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "set",
		Short: "Apply transformations on sets",
		Long:  `Apply transformations on sets such as splitting or changing their format`,
	...
	cmd.PersistentFlags().IntVarP(&(config.splitPercent), "split-percent", "p", 0, "percent of the set that will be dumped to the output")
...
func (scc *setCmdConfig) Validate() error {
	...
	if scc.splitPercent < 0 || scc.splitPercent > 100 {
		return fmt.Errorf("split-percent flag was set to an invalid value: it must be set to an integer between 0 and 100")
	}
	return nil
}
